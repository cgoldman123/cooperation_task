...
function FinalResults = Coop_fit(subject, subdat)
    rng('shuffle');
    ...
    BlockprobData = string(subdat.trial_type(subdat.event_type==3));
    BlockprobData = strrep(BlockprobData,'_',' ');
    ...
    sub.o = str2num(cell2mat(subdat.result(subdat.event_type == 5)));
    sub.u = str2num(cell2mat(subdat.response(subdat.event_type == 5)));
try
    for i = 1:N
        if sub.o(i) == 0 
            sub.o(i) = 3;
    ...
    MDP = Coop_gen_model(BlockProbs(1,:),beta,alpha,cr,eta_win,eta_loss,prior_a);
    ...
    DCM.pp     = [alpha,cr,beta,eta_win,eta_loss,prior_a];

    DCM        = Coop_inversion(DCM);   % Invert the model
    ...
    for block=1:NB
        sim_mdp = Coop_gen_model(BlockProbs(block, :), 1, posterior(1), posterior(2), posterior(3), posterior(4), posterior(5));
    ...
    FinalResults = [{subject} prior posterior DCM all_MDPs];
end
