function [MDP] = spm_MDP_VB_X_eta2(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_eta2(MDP(:,i),OPTIONS);
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_eta2(mdp);
...
for m = 1:size(MDP,1)
    ...
    for t = 1:T
        ...
        if isfield(MDP,'a')
            for g = 1:Ng(m)
                ...
                da     = da.*(MDP(m).a{g} > 0);
                if g==1
                    MDP(m).a{g} = (MDP(m).a{g}>.00001).*(MDP(m).a{g} - prior_a)*1 + (MDP(m).a{g}>.00001).*prior_a;
                end
            ...
            if g==1 && t==T %
              if sum(da(2,:))>0 %[this should be in da]
                MDP(m).a{g} = MDP(m).a{g} + da*eta_win;
              elseif sum(da(3,:))>0  %[this should be in da]
                MDP(m).a{g} =  MDP(m).a{g} + da*eta_loss;
              else
                MDP(m).a{g} = MDP(m).a{g} + da*eta; %if outcome is not win or loss, not sure if that's possible
              end
            %else
              %MDP(m).a{g} = MDP(m).a{g} + da*eta; %any other factor time point
            end
            end
        end
        ...
        if isfield(MDP,'b') && t > 1
            for f = 1:Nf(m)
                for k = 1:Np(m)
                    ...
                    db  = db.*(MDP(m).b{f}(:,:,v) > 0);
                    MDP(m).b{f}(:,:,v) = MDP(m).b{f}(:,:,v)*1 + db*eta;
                end
            end
        end
    ...
    if isfield(MDP,'d')
        for f = 1:Nf(m)
            i = MDP(m).d{f} > 0;
            MDP(m).d{f}(i) = MDP(m).d{f}(i)*1 + X{m,f}(i,1);
        end
    end
