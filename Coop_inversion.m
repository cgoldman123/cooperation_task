...
function [DCM] = Coop_inversion(DCM)
...
for i = 1:length(DCM.field)
    ...
    if ALL
        ...
        if strcmp(field,'alpha')
            pE.(field) = log(DCM.pp(1));               % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'beta')
            pE.(field) = log(DCM.pp(3));                % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'cs')
            pE.(field) = log(2^0);              % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'prior_a')
            pE.(field) = log(DCM.pp(6));              % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'cr')
            pE.(field) = log(DCM.pp(2));              % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'eta_win')
            pE.(field) = log(DCM.pp(4)/(1-DCM.pp(4)));      % in logit-space - bounded between 0 and 1!
            pC{i,i}    = prior_variance;
        elseif strcmp(field,'eta_loss')
            pE.(field) = log(DCM.pp(5)/(1-DCM.pp(5)));      % in logit-space - bounded between 0 and 1!
            pC{i,i}    = prior_variance;
        else
            pE.(field) = 0;      
            pC{i,i}    = prior_variance;
...
for idx_block = 1:mdp.NB
    ...
    mdp_block = Coop_gen_model(mdp.BlockProbs(idx_block,:),mdp.beta,mdp.alpha,mdp.cr,mdp.eta_win,mdp.eta_loss,mdp.prior_a);
